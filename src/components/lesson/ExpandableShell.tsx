...
export function ExpandableShell({
  ...
  allowExpand?: boolean;
  /** Capture the visible body (as currently zoomed/panned) as a PNG download. */
  ...
  const downloadSnapshot = async () => {
    ...
    try {
      ...
      a.href = dataUrl;
      a.download = `${(snapshotName || 'viewer').replace(/[^\w.\-]+/g, '_')}.png`;
      a.click();
    } catch {
      // iframe PDF plugins often can't be rasterized; still try outer shell
      try {
      ...
      <div className="flex items-center gap-1.5">
        {allowSnapshot && (
          <button
            ...
            disabled={capturing}
            className="inline-flex cursor-pointer items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-2.5 py-1 text-[11px] font-bold text-slate-700 shadow-sm hover:bg-slate-50 disabled:opacity-50 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-200"
            title="Download visible area as image"
          >
            <Download className="h-3.5 w-3.5" />
            <span className="text-[var(--lesson-accent,#0e6e6a)]">
              {capturing ? '…' : 'Download'}
            </span>
          </button>
        )}
